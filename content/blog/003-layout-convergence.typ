...
#figure({
  ...
  set text(0.70em, font: "FreeSans",)
  diagram(spacing: 5mm, node-shape: circle, {
    let node = node.with(radius: 1.1cm, stroke: 0.5pt)
    node((150deg, 1.5cm), name: <a>)[we need to render the outline, so ...]
    node((30deg,  1.5cm), name: <b>)[we need to render the headings, so ...]
    node((270deg, 1.5cm), name: <c>)[#move(dy: -0.3em)[we need to know what's before the headings, so ...]]
    edge(<a>, <b>, "->", bend: 30deg)
    edge(<b>, <c>, "->", bend: 30deg)
    edge(<c>, <a>, "->", bend: 30deg)
  })
...
In all of these cases, there is an underlying assumption: updating the document's outline won't change the information the outline is based on -- that is, the headings and the corresponding page numbers. Imagine this unlucky scenario:
...
Before we go on, I want to point out that the underlying problem is not specific to Typst. In both Word and LaTeX, updating the table of contents/recompiling your document will give you a small change if the problem is present, but no indicator _that_ it is there. Chances are, you will just not notice it.
...
- In "tree", the second visible update doesn't override the first one, but it again calculates a changed state. In total, the calculation here is 1+1+1.
...
arr.update(value => (..value, "first"))

// array is empty
context arr.get().first()
```
#{
}

It should not come as a surprise by now that in the code above, `arr.get()` will return `()`, and thus `().first()` will fail. This is a bit of a special case compared to the initial example: the value in the first iteration is not the final one, but it even results in an error. Typst will suppress errors that happen in earlier layout iterations, but with `layout-ltd` you may see them.
...
The "Layout did not converge within 5 attempts" warning can be caused by truly divergent document layouts, or layouts that simply need more iterations. In the latter case, the solution is to find the source of the extra iterations. I strongly recommend you don't ignore this warning, as it can have unpredictable impacts on how your document turns out.
